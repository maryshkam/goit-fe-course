...
galleryItems.forEach(val => {
  const item = document.createElement('li');
  gallery.append(item);
  const imgGallery = document.createElement('img');
  item.append(imgGallery);
  imgGallery.setAttribute("src", val.url);
  imgGallery.setAttribute("alt", val.alt);
  imgGallery.style.width = '300px';
  item.style.listStyle = 'none';
});
// const galleryLi = document.querySelector('.gallery').childNodes;
