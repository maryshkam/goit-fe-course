...
class Hamburger {
  ...
  calculatePrice() {
    ...
    let priceToppings = this._toppings.reduce((acc, el) => acc + Hamburger.TOPPINGS[el].price, 0 );
    let summ = priceSize + priceStuffing + priceToppings;
    return summ;
  }
   ...
   */
  calculateCalories() {
    let kalSize = Hamburger.SIZES[this._size].calories;
    let kalStuffing = Hamburger.STUFFINGS[this._stuffing].calories;
    let kalToppings = this._toppings.reduce((acc, el) => acc + Hamburger.TOPPINGS[el].calories, 0 );
    let summ = kalSize + kalStuffing + kalToppings;
    return summ;
  }
}
